...
export const tripsRouter = router({
  ...
  join: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: existingGroups } = await ctx.supabase
        .from('groups')
        .select(
          '*, steward_id, memberships:group_memberships(user_id, user:profiles(id, display_name))'
        )
      ...
      const { error: rebalanceError } = await ctx.supabase.rpc(
      ...
      if (rebalanceError) {
        ...
        if (
          ...
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message:
              'Your user profile is not properly set up. Please contact support.',
          });
        }

        if (
          errorMessage.includes('foreign key') &&
          errorMessage.includes('trips')
        ) {
          throw new TRPCError({
            code: 'NOT_FOUND',
  ...
  leave: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const typedCurrentGroups = (currentGroups ||
        []) as GroupWithMemberships[];
