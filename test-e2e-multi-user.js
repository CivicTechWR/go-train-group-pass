...
async function simulateUserJoining(userId, tripId) {
  ...
  try {
    ...
    if (data.result?.data) {
      ...
      console.error(`❌ User ${userId} failed to join trip:`, data.error?.message);
      return false;
    }
  } catch (error) {
...
async function testGroupFormation() {
  ...
  const testTrip = trips.find(trip => {
  ...
  console.log(`🎯 Testing with trip: ${testTrip.train.departure_time} on ${testTrip.date}`);
  ...
  console.log(`👤 Total Riders: ${finalTrip.groups.reduce((sum, group) => sum + group.memberships.length, 0)}`);
  
  finalTrip.groups.forEach((group, index) => {
    console.log(`\n📦 Group ${group.group_number}:`);
    group.memberships.forEach(membership => {
      console.log(`  - ${membership.user.display_name} (${membership.user_id})`);
    });
  });
  ...
  if (profiles) {
    for (const profile of profiles) {
      try {
        ...
        console.error(`❌ Failed to delete user ${profile.display_name}:`, error.message);
      }
    }
  }
}
