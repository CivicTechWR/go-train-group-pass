...
export class Migration20251030210056 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create table "calendar" ("service_id" varchar(255) not null, "monday" boolean not null, "tuesday" boolean not null, "wednesday" boolean not null, "thursday" boolean not null, "friday" boolean not null, "saturday" boolean not null, "sunday" boolean not null, "start_date" date not null, "end_date" date not null, constraint "calendar_pkey" primary key ("service_id"));`,
    );

    this.addSql(
      `create table "calendar_dates" ("service_id" varchar(255) not null, "date" date not null, "exception_type" int not null, constraint "calendar_dates_pkey" primary key ("service_id", "date"));`,
    );
    this.addSql(
    ...
    this.addSql(
      `create table "routes" ("route_id" varchar(255) not null, "agency_id" varchar(255) null, "route_short_name" varchar(255) not null, "route_long_name" varchar(255) not null, "route_desc" varchar(255) null, "route_type" int not null, "route_url" varchar(255) null, "route_color" varchar(255) null, "route_text_color" varchar(255) null, constraint "routes_pkey" primary key ("route_id"));`,
    );
    ...
    this.addSql(
      `alter table "trips" add constraint "trips_route_route_id_foreign" foreign key ("route_route_id") references "routes" ("route_id") on update cascade;`,
    );

    this.addSql(
      `alter table "stop_times" add constraint "stop_times_trip_trip_id_foreign" foreign key ("trip_trip_id") references "trips" ("trip_id") on update cascade;`,
    );
    this.addSql(
      `alter table "stop_times" add constraint "stop_times_stop_stop_id_foreign" foreign key ("stop_stop_id") references "stops" ("stop_id") on update cascade;`,
    );
  }
}
