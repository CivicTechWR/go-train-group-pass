...
export class Migration20251125082233 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "go-train-group-pass"."gtfs_routes" ("id" uuid not null default gen_random_uuid(), "route_id" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "route_short_name" varchar(255) not null, "route_long_name" varchar(255) not null, "route_desc" varchar(255) null, "route_type" int not null, "route_url" varchar(255) null, "route_color" varchar(255) null, "route_text_color" varchar(255) null, "agency_id" uuid null, "gtfsfeed_info_id" uuid not null, constraint "gtfs_routes_pkey" primary key ("id", "route_id"));`);
    ...
    this.addSql(`create table "go-train-group-pass"."gtfs_trips" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "trip_id" varchar(255) not null, "calendar_date_id" uuid not null, "trip_headsign" varchar(255) null, "trip_short_name" varchar(255) null, "direction_id" int null, "block_id" varchar(255) null, "shape_id" varchar(255) null, "wheelchair_accessible" int null, "bikes_allowed" int null, "route_id" uuid not null, "route_route_id" varchar(255) not null, "gtfsfeed_info_id" uuid not null, constraint "gtfs_trips_pkey" primary key ("id"));`);
    this.addSql(`create index "idx_trips_calendar_date" on "go-train-group-pass"."gtfs_trips" ("calendar_date_id");`);
    this.addSql(`create index "idx_trips_route" on "go-train-group-pass"."gtfs_trips" ("route_id", "route_route_id");`);

    this.addSql(`create table "go-train-group-pass"."gtfs_stop_times" ("id" uuid not null default gen_random_uuid(), "stop_time_id" varchar(255) not null, "stop_sequence" int not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "arrival_time" varchar(8) not null, "departure_time" varchar(8) not null, "stop_headsign" varchar(255) null, "pickup_type" int null, "drop_off_type" int null, "shape_dist_traveled" numeric(10,0) null, "timepoint" int null, "stop_id" uuid not null, "trip_id" uuid not null, "gtfsfeed_info_id" uuid not null, constraint "gtfs_stop_times_pkey" primary key ("id", "stop_time_id", "stop_sequence"));`);
    ...
    this.addSql(`alter table "go-train-group-pass"."gtfs_trips" add constraint "gtfs_trips_route_id_route_route_id_foreign" foreign key ("route_id", "route_route_id") references "go-train-group-pass"."gtfs_routes" ("id", "route_id") on update cascade;`);
