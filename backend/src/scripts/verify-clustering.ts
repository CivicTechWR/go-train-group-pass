...
async function verify() {
  ...
  let soloBookings = 0;
  const totalTrips = (results as Array<{ count: number }>).length;
  let groupedTrips = 0;

  for (const row of results as Array<{ count: number }>) {
  ...
  const groupedPercentage =
    totalBookings > 0 ? (groupedBookings / totalBookings) * 100 : 0;
